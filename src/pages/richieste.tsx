...
const menuItems = [
  ...
  { label: "Impostazioni", icon: Settings, active: false, path: "#" },
];
          ...
          <div className="rounded-[26px] border border-zinc-900 bg-[#121316] p-5 sm:p-6">
            ...
            {tab === "incoming" && (
              <div className="space-y-4">
                {incoming.length === 0 && (
                  <p className="text-zinc-500 text-center py-16 text-sm">Nessuna richiesta in arrivo.</p>
                )}
                {incoming.map((req) => (
                  <div
                    key={req.id}
                    className="flex items-center justify-between bg-[#17181b] border border-zinc-800 hover:border-orange-500/30 transition rounded-2xl p-4"
                  >
                    <div className="flex items-center gap-4">
                      ...
                      />
                      <div>
                        <h3 className="font-semibold text-white">{req.from_user_name || "Utente"}</h3>
                    ...
                    <div className="flex items-center gap-3">
                      <button
                        ...
                        className="px-4 py-2 rounded-xl border border-green-500/40 bg-green-500/10 text-green-400 hover:bg-green-500/20 transition flex items-center gap-2 text-sm"
                      >
                        <Check size={16} />
                        Accetta
                      </button>
                      <button
                        ...
                        className="px-4 py-2 rounded-xl border border-red-500/40 bg-red-500/10 text-red-400 hover:bg-red-500/20 transition flex items-center gap-2 text-sm"
                      >
                        <X size={16} />
                        Rifiuta
                      </button>
            ...
            {tab === "outgoing" && (
              <div className="space-y-4">
                {outgoing.length === 0 && (
                  <p className="text-zinc-500 text-center py-16 text-sm">Nessuna richiesta inviata.</p>
                )}
                {outgoing.map((req) => (
                  <div
                    key={req.id}
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      />
                      <div>
                        <h3 className="font-semibold text-white">{req.to_user_name || "Utente"}</h3>
                        <p className="text-xs text-zinc-500">{new Date(req.created_at).toLocaleDateString()}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      {req.status === "accepted" && (
                        <button
                          ...
                          className="px-4 py-2 rounded-xl bg-orange-500/20 border border-orange-500/40 text-orange-400 hover:bg-orange-500/30 transition flex items-center gap-2 text-sm"
                        >
                          <Check size={16} />
                          Completa
                        </button>
                      ...
                      {req.status === "pending" && (
                        <button
                          ...
                          className="px-4 py-2 rounded-xl border border-red-500/40 bg-red-500/10 text-red-400 hover:bg-red-500/20 transition text-sm"
                        >
                          Cancella
                        </button>
                      )}
