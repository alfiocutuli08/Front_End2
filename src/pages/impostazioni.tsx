...
export default function Impostazioni() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await authService.updateProfile({ name, bio, location, level });
      setSaved(true);
      ...
      <div className="mx-auto flex min-h-[calc(100svh-1.5rem)] w-full max-w-[1280px] overflow-hidden rounded-[28px] border border-orange-500/15 bg-[#0b0b0c] shadow-[0_24px_80px_rgba(0,0,0,0.45)]">
        ...
        <section className="flex-1 bg-[#101113] p-5 sm:p-7">
          <div className="rounded-[26px] border border-zinc-900 bg-[#121316] p-5 sm:p-6 space-y-8">
            <h1 className="m-0 text-left text-[2rem] font-semibold leading-tight tracking-[-0.03em] text-white sm:text-[2.3rem]">Impostazioni</h1>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-[#17181b] border border-zinc-800 rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-orange-500/60" />
                </div>
                <div>
                  <label className="text-xs text-zinc-500 mb-1 block">Città</label>
                  <input value={location} onChange={(e) => setLocation(e.target.value)} className="w-full bg-[#17181b] border border-zinc-800 rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-orange-500/60" />
                </div>
                <div className="sm:col-span-2">
                  <label className="text-xs text-zinc-500 mb-1 block">Bio</label>
                  <textarea value={bio} onChange={(e) => setBio(e.target.value)} rows={3} className="w-full bg-[#17181b] border border-zinc-800 rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-orange-500/60 resize-none" />
                </div>
                <div>
                  <label className="text-xs text-zinc-500 mb-1 block">Livello</label>
                  <select value={level} onChange={(e) => setLevel(e.target.value)} className="w-full bg-[#17181b] border border-zinc-800 rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-orange-500/60">
              ...
              <button onClick={handleSave} disabled={saving} className="mt-5 flex items-center gap-2 px-6 py-3 rounded-2xl bg-orange-500 text-black font-bold hover:bg-orange-400 transition disabled:opacity-50">
                <Save size={18} />
            ...
            <div className="border-t border-zinc-800 pt-8">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="w-full bg-[#17181b] border border-zinc-800 rounded-xl px-4 py-3 text-sm text-zinc-400">{user?.email ?? "—"}</div>
                </div>
                <button onClick={handleLogout} className="flex items-center gap-2 px-6 py-3 rounded-2xl border border-red-500/40 bg-red-500/10 text-red-400 font-bold hover:bg-red-500/20 transition">
                  <LogOut size={18} /> Esci
                </button>
              </div>
            </div>
